...
import { useGetAllCategoriesQuery } from '@/lib/store';
import type { Category } from '@/lib/types';

function toCardProps(c: Category) {
  return {
    id: c.id,
    name: c.name,
    slug: c.slug,
  ...
  return (
    <div className="relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
        {[
          { label: 'Total Collections', value: isLoading ? '…' : String(allCategories.length) },
      ...
      <AddCollectionPanel isOpen={isPanelOpen} onClose={() => setIsPanelOpen(false)} />
    </div>
  );
}
