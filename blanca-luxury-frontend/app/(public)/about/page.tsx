...
export default function AboutPage() {
  ...
  // Auto-scroll: 40px/s, pauses on hover/drag, stops at end
  useEffect(() => {
    const el = timelineRef.current;
    if (!el) return;

    // Only run when the section is actually overflowing (narrow screens)
    const obs = new IntersectionObserver(([entry]) => {
      if (!entry.isIntersecting) return;
      obs.disconnect();

      // Wait for milestones to animate in, then start scrolling
  ...
  function onMouseLeave() { isHovered.current = false; isDragging.current = false; }
    ...
    <main className="overflow-x-hidden">
      ...
      <section className="min-h-[80svh] w-full flex flex-col items-center justify-center bg-[#fff8f1] px-6 py-24 text-center relative">
        ...
        <ScrollReveal delay={700} y={16} duration={900}>
          <div className="mt-14 h-pxw-15 bg-[#C9A96E] mx-auto" />
        </ScrollReveal>

        {/* Scroll indicator */}
        <div className="absolute bottom-12 flex flex-col items-center gap-3">
          <span className="font-sans text-[9px] uppercase tracking-[0.25em] text-secondary">Scroll</span>
            ...
            <div className="flex flex-wrap items-center gap-6 md:gap-8 mt-14">
              {[
                ...
                <div key={icon} className="flex flex-col items-center gap-2 group cursor-default">
                  ...
                  <span className="font-sans text-[8px] uppercase tracking-widest text-secondary/50 group-hover:text-[#745a27] transition-colors duration-300 hidden md:block">
                    {label}
                  </span>
                </div>
              ))}
      ...
      <section className="bg-[#1A1410] py-32 overflow-hidden">
        ...
        >
          <div className="relative flex items-center min-w-[1200px] h-72">
            ...
            <div className="absolute top-1/2 left-0 w-full h-pxbg-[#C9A96E]/25 z-0" />
      ...
      <section className="flex flex-col md:flex-row w-full" style={{ minHeight: "540px" }}>
        ...
        <div className="w-full md:w-1/2 relative group overflow-hidden" style={{ minHeight: "440px" }}>
          <img
            ...
            className="w-full h-full object-cover transition-transform duration-1200 ease-out scale-105 group-hover:scale-100"
      ...
      <div className="bg-[#1A1410] py-16 px-8 text-center">
        <ScrollReveal y={24} delay={100}>
          ...
          <div className="mt-6 h-pxw-15 bg-[#C9A96E] mx-auto" />
        </ScrollReveal>
      </div>
