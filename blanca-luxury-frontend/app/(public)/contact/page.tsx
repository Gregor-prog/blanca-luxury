      ...
      <span
        className={`px-4 py-2 rounded-full border text-[11px] font-sans uppercase tracking-widest block transition-all duration-300 ${
          checked
            ...
            : "border-outline-variant/60 text-secondary hover:border-[#C9A96E] hover:text-[#745a27]"
        }`}
      >
        {label}
      </span>
...
function ShowroomCard({ showroom }: { showroom: Showroom }) {
  const mapsUrl = `https://maps.google.com/?q=${encodeURIComponent(showroom.address + " " + showroom.city)}`;
  ...
  return (
    <div className="bg-white border border-[#E8E0D5] p-10 flex flex-col gap-6">
      <span className="font-sans text-[10px] tracking-[0.35em] text-[#C9A96E] uppercase">
        {showroom.city} Showroom{showroom.city.toLowerCase() === "port harcourt" ? "s" : ""}
      </span>
      ...
      <div className="flex flex-col gap-1">
        {showroom.phoneNumbers.map((phone) => (
          <p key={phone} className="font-sans text-[13px] text-[#8B7D72]">
            {phone}
          </p>
        ))}
        {showroom.email && (
          <p className="font-sans text-[13px] text-[#8B7D72]">{showroom.email}</p>
        )}
      </div>
...
function ShowroomSkeleton() {
  return (
    <div className="bg-white border border-[#E8E0D5] p-10 flex flex-col gap-6 animate-pulse">
      <div className="h-3 w-32 bg-[#E8E0D5] rounded" />
      <div className="h-12 w-3/4 bg-[#E8E0D5] rounded" />
      <div className="space-y-2">
        <div className="h-3 w-40 bg-[#E8E0D5] rounded" />
        <div className="h-3 w-48 bg-[#E8E0D5] rounded" />
      </div>
      <div className="h-3 w-24 bg-[#E8E0D5] rounded" />
    </div>
  );
}
...
export default function ContactPage() {
  ...
  // Cached for 10 min — showrooms almost never change
  const { data: showroomsData, isLoading: showroomsLoading } =
  ...
  return (
    <main>
      {/* PAGE HERO */}
      <section className="relative w-full h-90 flex flex-col items-center justify-center overflow-hidden">
        <img
        ...
        <div className="relative z-20 text-center px-4 pt-20">
          ...
          <ScrollReveal delay={650} y={20}>
            <p className="font-sans text-[13px] tracking-widest text-tertiary-container uppercase">
          ...
          <div className="max-w-md mx-auto md:mx-0">
            ...
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-8">
              ...
              <ScrollReveal delay={80} y={24}>
                <div className="border-b border-outline-variant/40 focus-within:border-[#745a27] transition-colors duration-300 pb-2">
                  <label className="block font-sans text-[10px] tracking-widest text-secondary uppercase mb-2">
                    Full Name
                  </label>
                  <input
                    type="text"
                    ...
                    required
                    className="w-full bg-transparent border-none p-0 focus:ring-0 font-body text-[#1e1b15] placeholder:text-outline/40 outline-none text-[15px]"
                  />
                </div>
              </ScrollReveal>
              ...
              <ScrollReveal delay={260} y={24}>
                <div>
                  <label className="block font-sans text-[10px] tracking-widest text-secondary uppercase mb-4">
                    Location Interest
                  </label>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="relative border-b border-outline-variant/40 focus-within:border-[#745a27] transition-colors duration-300 pb-2">
                      ...
                      <span className="material-symbols-outlined absolute right-0 bottom-2 text-[16px] text-outline pointer-events-none">
                        expand_more
                      </span>
                    </div>
                    <div className="relative border-b border-outline-variant/40 focus-within:border-[#745a27] transition-colors duration-300 pb-2">
              ...
              <ScrollReveal delay={380} y={24}>
                <div className="border-b border-outline-variant/40 focus-within:border-[#745a27] transition-colors duration-300 pb-2">
                  <textarea
                    ...
                    className="w-full bg-transparent border-none p-0 focus:ring-0 font-serif italic text-[17px] text-[#1e1b15] placeholder:text-outline/40 outline-none resize-none leading-relaxed"
                  />
                </div>
              </ScrollReveal>
              ...
              <ScrollReveal delay={440} y={20}>
                ...
                >
                  {sending ? (
                    <>
                      <svg className="w-4 h-4 animate-spin text-[#C9A96E]" viewBox="0 0 24 24" fill="none">
        ...
        <section className="w-full md:w-1/2 bg-[#F0EBE3] px-8 md:px-16 lg:px-24 py-20 md:py-28 flex flex-col gap-8">
          {showroomsLoading ? (
            ...
            showrooms.map((showroom, i) => (
              <ScrollReveal key={showroom.id} delay={100 + i * 150} y={32}>
                <ShowroomCard showroom={showroom} />
              </ScrollReveal>
            ))
          ) : (
            /* Fallback: hardcoded if API is down */
            <>
        ...
        <ScrollReveal delay={0} y={24}>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a
              ...
              className="bg-transparent border border-outline-variant/25 text-white px-10 py-4 rounded-full font-sans text-[12px] uppercase tracking-widest hover:bg-white/5 active:scale-95 transition-all duration-300 flex items-center justify-center gap-2"
            >
