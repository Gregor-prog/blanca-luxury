...
export function ShowroomCard({ showroom }: { showroom: ShowroomProps }) {
  return (
    <article className="bg-admin-surface border border-admin-border rounded-[8px] relative overflow-hidden group hover:border-admin-gold/30 transition-all duration-300">
      {/* Cover Image */}
      <div className="relative w-full h-[140px] bg-admin-surface-elevated overflow-hidden">
        {showroom.coverImageUrl ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="material-symbols-outlined text-[40px] text-admin-border">store</span>
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-admin-surface/80 via-transparent to-transparent" />
        {/* Top Accent */}
        <div className="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-admin-gold/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
      </div>
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${showroom.isActive ? 'bg-admin-success' : 'bg-admin-text-muted'} shadow-[0_0_8px_rgba(76,175,125,0.4)]`} />
